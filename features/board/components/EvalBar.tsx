...
function scoreToWhite(cp: number | null, mate: number | null, turn: "w" | "b") {
  if (mate !== null) {
    const huge = 1000;
...
function formatEvalFromWhiteScore(whiteScore: number, mate: number | null) {
  if (mate !== null) {
...
export function EvalBar({ fen, depth = 12 }: EvalBarProps) {
  ...
  const whitePercent = clamp(((compressedScore + 1000) / 2000) * 100, 0, 100);
  ...
  return (
    <div className="flex flex-col items-center gap-1 select-none w-0 h-full">
      ...
      >
        <div className="absolute left-0 top-1/2 w-full h-px bg-gray-500 z-10 -translate-y-1/2"></div>
